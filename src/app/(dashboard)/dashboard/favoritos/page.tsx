...
function FavoriteCard({ favorite }: { favorite: FavoriteProfessional }) {
  ...
  return (
    <Link href={`/profissionais/${professional.slug}`} className="client-card flex gap-4 p-4 no-underline">
      <div className="h-20 w-20 shrink-0 overflow-hidden rounded-[14px] border border-[#ca4651]/20 bg-white/[0.04]">
        {cover ? (
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="mt-3 text-[12px] font-black uppercase text-[#f2c8cc]">
          Ver perfil
        </p>
      </div>
      <ChevronRight className="mt-7 h-5 w-5 shrink-0 text-[#f2c8cc]" />
    </Link>
  );
}
