  ...
  return createPortal(
    <div className="client-filter-overlay" role="dialog" aria-modal="true" aria-label="Filtrar perfis">
      ...
      <section data-client-filter-panel="true" className="client-filter-panel">
        <div className="mx-auto mb-5 h-1.5 w-14 rounded-full bg-[#d77bff]/28" />
  ...
  return (
    <>
      ...
      <section className="client-explore-home">
        {/* Header */}
        <div>
          <p className="text-[14px] font-bold uppercase tracking-wide text-[#d77bff]">EXPLORAR PERFIS</p>
          <h1 className="mt-2 text-[40px] font-black leading-[1.04] tracking-[-0.03em] text-white">
            {city ? (
              <>
                Acompanhantes{" "}
                <span className="text-[#d77bff]">em {city}</span>
              </>
            ) : (
              <>
                Perfis <span className="text-[#d77bff]">verificados</span>
                <br />perto de você
              </>
            )}
          </h1>
        ...
        <div className="relative z-10 -mt-6 rounded-[28px] border border-white/[0.10] bg-white/[0.055] p-5 shadow-[0_22px_70px_rgba(0,0,0,0.38)] backdrop-blur-xl">
          <div className="mb-4 flex items-center justify-between gap-3">
            <h2 className="text-[15px] font-black uppercase text-[#d77bff]">CIDADE</h2>
            {city && (
              <button
                type="button"
          ...
          >
            <MapPin className="h-5 w-5 shrink-0 text-[#d77bff]" />
        ...
        {/* Categories */}
        <div className="mt-6">
          <p className="mb-3 text-[15px] font-black uppercase text-[#d77bff]">CATEGORIA</p>
        ...
        {/* Active filter tags */}
        {hasFilters && (
          <div className="mt-4 rounded-[18px] border border-[#d77bff]/18 bg-[#d77bff]/10 px-4 py-3">
            <div className="flex items-center justify-between gap-3">
              <span className="min-w-0 text-[13px] font-semibold text-white/72">
                Filtros ativos
              </span>
              <button type="button" onClick={clearFilters} className="shrink-0 text-[13px] font-black text-[#d77bff]">
                Limpar
              </button>
            </div>
            {activeFilterLabels.length > 0 && (
              <div className="mt-3 flex flex-wrap gap-2">
                {activeFilterLabels.map((label) => (
                  <span key={label} className="rounded-full border border-[#d77bff]/18 bg-black/20 px-2.5 py-1 text-[11px] font-bold text-[#f5f0e4]/72">
                    {label}
                  </span>
                ))}
              </div>
