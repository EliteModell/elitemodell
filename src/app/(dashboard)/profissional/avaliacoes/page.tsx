...
export default function ProfissionalAvaliacoesPage() {
  ...
  const card = { background: "#111", border: "1px solid rgba(202, 70, 81,.16)", borderRadius: 18, padding: 20 } as const;
          ...
          {reviews.map((review) => (
            <article key={review.id} style={card}>
              <div style={{ display: "flex", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
                ...
                <div style={{ color: "#ca4651", fontWeight: 900 }}>{"★".repeat(review.rating)}<span style={{ color: "#333" }}>{"★".repeat(5 - review.rating)}</span></div>
              </div>
              <p style={{ color: "#aaa", lineHeight: 1.65, margin: "14px 0" }}>{review.comment}</p>
              {review.dispute ? (
                <div style={{ border: "1px solid rgba(202, 70, 81,.22)", background: "rgba(202, 70, 81,.07)", borderRadius: 10, padding: 12, color: "#f2c8cc", fontSize: 13 }}>
                ...
                <div style={{ display: "grid", gap: 10 }}>
                  ...
                  <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                    <button disabled={saving} onClick={() => disputeReview(review.id)} style={{ padding: "10px 16px", background: "#ca4651", color: "#080704", border: 0, borderRadius: 10, fontWeight: 900, cursor: saving ? "wait" : "pointer" }}>Enviar contestação</button>
                ...
                <button onClick={() => setOpenId(review.id)} style={{ padding: "10px 14px", background: "transparent", border: "1px solid rgba(202, 70, 81,.28)", borderRadius: 10, color: "#ca4651", fontWeight: 800, cursor: "pointer" }}>
                  Contestar avaliação
                </button>
              )}
            </article>
