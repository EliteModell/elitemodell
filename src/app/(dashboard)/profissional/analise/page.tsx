...
export default async function ProfessionalAnalysisPage() {
  ...
  return (
    <div className="mx-auto grid min-h-[calc(100dvh-80px)] w-full max-w-[560px] place-items-center px-4 py-10">
      <section className="w-full rounded-[8px] border border-[#ca4651]/25 bg-[#101010] p-6 text-center shadow-[0_24px_80px_rgba(0,0,0,0.34)] sm:p-8">
        <div className={`mx-auto grid h-16 w-16 place-items-center rounded-[8px] border text-2xl font-black ${copy.tone === "danger" ? "border-[#ef4444]/35 bg-[#ef4444]/10 text-[#ff9aa4]" : "border-[#ca4651]/35 bg-[#ca4651]/12 text-[#f2c8cc]"}`}>
          {copy.tone === "danger" ? "!" : "..."}
        </div>

        <p className="mt-6 text-[11px] font-black uppercase tracking-[0.22em] text-[#ca4651]">Status profissional</p>
        ...
        <div className="mt-6 grid gap-3 rounded-[8px] border border-white/10 bg-white/[0.035] p-4 text-left">
          {[
            ...
            <p key={item} className="flex gap-3 text-sm leading-6 text-white/60">
              <span className="mt-1 h-2 w-2 shrink-0 rounded-full bg-[#ca4651]" />
              <span>{item}</span>
            </p>
          ))}
        </div>
        ...
        <div className="mt-7 flex flex-col gap-3 sm:flex-row">
          ...
          <Link href="mailto:suporte@elitemodell.com.br" className="inline-flex min-h-11 flex-1 items-center justify-center rounded-[8px] bg-[#ca4651] px-4 text-sm font-black text-[#080704] no-underline">
            Falar com suporte
          </Link>
        </div>
      </section>
