  ...
  return (
    <article style={{ border: "1px solid rgba(183,44,255,0.18)", borderRadius: 8, background: "rgba(255,255,255,0.035)", padding: 16 }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
        ...
        <div style={{ width: 42, height: 42, display: "grid", placeItems: "center", borderRadius: 8, color: GOLD, background: "rgba(183,44,255,0.10)", border: "1px solid rgba(183,44,255,0.20)" }}>
          {icon}
        </div>
      </div>
    </article>
  ...
  return (
    <div style={{ display: "grid", gap: 24 }}>
      <section style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 18, alignItems: "stretch" }}>
        <div style={{ border: "1px solid rgba(183,44,255,0.22)", borderRadius: 8, background: "linear-gradient(145deg, rgba(20,20,20,0.98), rgba(7,7,8,0.98))", padding: 22 }}>
          ...
          {!access.hostApproved ? (
            <div style={{ marginTop: 18, border: `1px solid ${GOLD}55`, borderRadius: 8, background: "rgba(183,44,255,0.10)", padding: 14, color: "#e1a6ff", fontSize: 13, lineHeight: 1.6 }}>
              Seu acesso está aberto para acompanhamento. Imóveis só aparecem para profissionais depois de aprovação administrativa.
            </div>
          ) : null}
        </div>

        <div style={{ border: "1px solid rgba(183,44,255,0.18)", borderRadius: 8, background: "rgba(255,255,255,0.035)", padding: 18, display: "grid", alignContent: "space-between", gap: 16 }}>
          <div>
      ...
      <section style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(320px, 1fr))", gap: 18 }}>
        <div style={{ display: "grid", gap: 12 }}>
          ...
          {properties.length === 0 ? (
            <div style={{ border: "1px dashed rgba(183,44,255,0.28)", borderRadius: 8, padding: 24, textAlign: "center", background: "rgba(255,255,255,0.025)" }}>
            ...
            <div style={{ display: "grid", gap: 12 }}>
              {properties.map((property) => {
                ...
                return (
                  <Link key={property.id} href={`/anfitriao/imoveis/${property.id}`} style={{ display: "grid", gridTemplateColumns: "96px minmax(0,1fr)", gap: 14, padding: 12, border: "1px solid rgba(183,44,255,0.16)", borderRadius: 8, background: "rgba(255,255,255,0.035)", textDecoration: "none" }}>
        ...
        <div style={{ display: "grid", gap: 12, alignSelf: "start" }}>
          ...
          {bookings.length === 0 ? (
            <div style={{ border: "1px solid rgba(183,44,255,0.16)", borderRadius: 8, background: "rgba(255,255,255,0.035)", padding: 18 }}>
              <p style={{ color: "#9ca3af", margin: 0 }}>Ainda não há reservas para seus imóveis.</p>
            </div>
          ) : (
            bookings.map((booking) => (
              <article key={booking.id} style={{ border: "1px solid rgba(183,44,255,0.16)", borderRadius: 8, background: "rgba(255,255,255,0.035)", padding: 14 }}>
