...
type ApiSuggestion = { placeId: string; mainText: string; secondaryText: string };
type CityChoice = { id: string; city: string; state: string; label: string; detail: string };
...
export default function HomeCitySearch() {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      try {
        const response = await fetch(`/api/address/search?input=${encodeURIComponent(input.trim())}`, { signal: controller.signal });
    ...
    <div className={`${styles.control} ${focused ? styles.focused : ""}`}>
      <input id="home-city-search" type="search" value={input} placeholder="Digite sua cidade" autoComplete="off" role="combobox"
        ...
        onChange={(event) => {
          ...
          else { setLoading(false); setRemoteChoices([]); }
        }}/>
