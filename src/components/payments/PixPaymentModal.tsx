...
export default function PixPaymentModal({ planId, creditAmount, bookingId, amount, onClose, onSuccess }: Props) {
  ...
  const modal = (
    <div
      ...
      aria-modal="true"
    >
      <div className="payment-light-modal relative w-full max-w-[400px] overflow-hidden rounded-[24px] border border-[#ca4651]/25 bg-white shadow-[0_28px_90px_rgba(37, 31, 32,0.24)]">
        <div className="h-[2px] bg-[linear-gradient(90deg,transparent,#df828a,#ca4651,transparent)]" />

        {/* Header */}
        <div className="flex items-center justify-between px-6 pt-5 pb-3">
          <p className="text-[13px] font-bold uppercase tracking-widest text-[#ca4651]">Pagar com PIX</p>
          {stage !== "paid" && (
            <button
              type="button"
              onClick={onClose}
        ...
        {stage === "loading" && (
          <div className="flex flex-col items-center gap-4 px-6 pb-8 pt-4">
            <Loader className="h-10 w-10 animate-spin text-[#ca4651]" />
        ...
        {stage === "waiting" && pix && (
          <div className="flex flex-col items-center gap-4 px-6 pb-7">
            ...
            {pix.qrCodeBase64 ? (
              <div className="rounded-[12px] border border-[#ca4651]/20 bg-white p-3">
                <img
