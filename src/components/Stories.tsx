            ...
            <div style={{ position: "absolute", top: 8, left: 8, right: 8, display: "flex", gap: 3, zIndex: 3 }}>
              {aberto.grupo.stories.map((s, i) => (
                <div key={s.id} style={{ flex: 1, height: 2, background: "#333", borderRadius: 2 }}>
                  <div style={{ height: "100%", borderRadius: 2, background: "linear-gradient(90deg,#b72cff,#cc0000)", width: i < aberto.idx ? "100%" : i === aberto.idx ? `${progresso}%` : "0%", transition: i === aberto.idx ? "width 0.05s linear" : "none" }} />
                </div>
              ))}
            </div>
