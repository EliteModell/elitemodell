...
export default function ActionAuthModal({ open, actionLabel, returnTo, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-[950] grid place-items-center bg-black/55 p-4 backdrop-blur-lg" role="dialog" aria-modal="true" aria-labelledby="action-auth-title">
      <section className="w-full max-w-md rounded-2xl border border-[#fcf7ff] bg-white p-6 text-[#141212] shadow-[0_32px_100px_rgba(37, 31, 32,.24)]">
        <div className="flex items-start justify-between gap-4">
          ...
          <button type="button" onClick={onClose} aria-label="Fechar" className="grid h-10 w-10 shrink-0 place-items-center rounded-full border border-[#fcf7ff] bg-[#f7f7fa] text-[#514b59]">
            <X size={18} />
          </button>
        </div>
        ...
        <div className="mt-6 grid grid-cols-2 gap-3">
          <Link href={loginHref} className="grid min-h-12 place-items-center rounded-xl bg-[#b72cff] px-4 text-sm font-black text-white no-underline">
            Entrar
          </Link>
          <Link href={cadastroHref} className="grid min-h-12 place-items-center rounded-xl border border-[#b72cff]/35 bg-[#b72cff]/[.07] px-4 text-sm font-black text-[#8f1fd1] no-underline">
            Criar conta
          </Link>
        </div>
      </section>
