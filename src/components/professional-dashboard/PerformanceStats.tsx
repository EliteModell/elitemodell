  ...
  return (
    <section className="premium-lower-card p-5 sm:p-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <p className="text-[11px] font-black uppercase tracking-[0.22em] text-[#b72cff]">Desempenho do perfil</p>
          ...
          {(Object.keys(periodLabels) as PerformancePeriod[]).map((key) => (
            <button
              ...
              className={`min-h-10 rounded-[12px] px-2 text-xs font-black transition ${
                period === key ? "bg-[#b72cff] text-[#080704]" : "text-white/52 hover:bg-white/[0.06] hover:text-white"
              }`}
            >
              {periodLabels[key]}
            </button>
        ...
        {stats.map((stat) => {
          ...
          return (
            <div key={stat.label} className="min-h-[140px] rounded-[18px] border border-[#b72cff]/18 bg-[linear-gradient(145deg,rgba(255,255,255,0.045),rgba(183,44,255,0.025))] p-4">
              ...
              <div className="mt-2 text-[11px] font-black uppercase tracking-[0.12em] text-[#e1a6ff]/70">{stat.note}</div>
            </div>
          );
        })}
      </div>
