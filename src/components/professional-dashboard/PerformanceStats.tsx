  ...
  return (
    <section className="premium-lower-card p-5 sm:p-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <p className="text-[11px] font-black uppercase tracking-[0.22em] text-[#ca4651]">Desempenho do perfil</p>
          ...
          {(Object.keys(periodLabels) as PerformancePeriod[]).map((key) => (
            <button
              ...
              className={`min-h-10 rounded-[12px] px-2 text-xs font-black transition ${
                period === key ? "bg-[#ca4651] text-[#080704]" : "text-white/52 hover:bg-white/[0.06] hover:text-white"
              }`}
            >
              {periodLabels[key]}
            </button>
      ...
      <div className="mt-6 grid grid-cols-2 gap-4 lg:grid-cols-3 xl:grid-cols-6">
        {stats.map((stat) => {
          ...
          return (
            <div key={stat.label} className="min-h-[140px] rounded-[18px] border border-[#ca4651]/18 bg-[linear-gradient(145deg,rgba(255,255,255,0.045),rgba(202, 70, 81,0.025))] p-4">
              <div className="mb-4 grid h-10 w-10 place-items-center rounded-[14px] border border-[#ca4651]/24 bg-[#ca4651]/10 text-[#f2c8cc]">
              ...
              <div className="mt-2 text-[11px] font-black uppercase tracking-[0.12em] text-[#f2c8cc]/70">{stat.note}</div>
            </div>
          );
        })}
      </div>
