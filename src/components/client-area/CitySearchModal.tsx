      ...
      <div className="relative mx-auto flex min-h-dvh max-w-[820px] flex-col px-4 pb-[calc(env(safe-area-inset-bottom)+38px)] pt-[calc(env(safe-area-inset-top)+22px)] sm:px-8">
        <header className="flex items-center justify-between">
          ...
          <button
            type="button"
            onClick={onClose}
            className="grid h-[66px] w-[66px] place-items-center rounded-[14px] border border-[#d77bff]/35 bg-[#080808]/65 text-[#d77bff] shadow-[0_0_28px_rgba(183, 44, 255,0.12)] transition hover:bg-[#141212] active:scale-95"
            aria-label="Fechar"
          >
        ...
        <section className="relative mt-[66px] min-h-[298px] overflow-visible">
          ...
          <h1 className="relative mt-6 text-[64px] font-black leading-[0.98] tracking-[-0.04em] text-[#fffaf0] [text-shadow:0_8px_34px_rgba(0,0,0,0.78)] sm:text-[72px]">
            Selecionar<br />cidade <span className="text-[#d77bff]">✦</span>
          </h1>
        ...
        <section className="relative mt-[28px]">
          <div className="flex h-[94px] items-center rounded-[14px] border border-[#b72cff]/85 bg-[#07090a]/92 shadow-[0_0_0_1px_rgba(183, 44, 255,0.08),0_22px_60px_rgba(0,0,0,0.46),inset_0_1px_0_rgba(255,255,255,0.045)] transition focus-within:border-[#d77bff] focus-within:shadow-[0_0_0_1px_rgba(183, 44, 255,0.20),0_0_34px_rgba(183, 44, 255,0.11)]">
            <div className="grid h-full w-[86px] shrink-0 place-items-center rounded-l-[13px] border-r border-[#b72cff]/30 bg-[#080808]/82">
              <MapPin className="h-12 w-12 text-[#d77bff] drop-shadow-[0_0_16px_rgba(183, 44, 255,0.35)]" strokeWidth={2.2} />
            </div>
            <input
              ref={inputRef}
              type="text"
            ...
            <div className="grid w-[74px] shrink-0 place-items-center">
              {busy ? (
                <div className="h-7 w-7 animate-spin rounded-full border-[3px] border-[#d77bff] border-t-transparent" />
              ) : input ? (
                <button
                  type="button"
                  onClick={reset}
                  className="grid h-10 w-10 place-items-center rounded-full bg-white/8 text-[#d77bff] transition active:bg-white/14"
                  aria-label="Limpar busca"
                >
                  <X className="h-5 w-5" />
                </button>
              ) : (
                <Search className="h-10 w-10 text-[#d77bff]" strokeWidth={2.2} />
              )}
            </div>
          </div>
        </section>
        ...
        <main className="relative mt-[74px] flex-1">
          {noResults && !checking && (
            <div className="flex flex-col items-center rounded-[18px] border border-[#d77bff]/20 bg-white/[0.035] px-6 py-12 text-center shadow-[0_24px_80px_rgba(0,0,0,0.35)]">
              <div className="mb-6 grid h-[88px] w-[88px] place-items-center rounded-[24px] border border-[#d77bff]/35 bg-[#080808]/82 shadow-[0_0_46px_rgba(183, 44, 255,0.14)]">
                <Clock className="h-9 w-9 text-[#d77bff]" />
              </div>
              <p className="text-[11px] font-black uppercase tracking-[0.24em] text-[#d77bff]">Em breve</p>
              ...
              <button
                type="button"
                onClick={reset}
                className="mt-8 flex min-h-12 items-center gap-2 rounded-[14px] border border-[#d77bff]/45 bg-[#d77bff]/14 px-7 text-[14px] font-bold text-[#d77bff] transition active:scale-95"
              >
          ...
          {!noResults && !busy && suggestions.length > 0 && (
            <ul className="overflow-hidden rounded-[18px] border border-[#d77bff]/18 bg-white/[0.035]" role="listbox">
              {suggestions.map((s, i) => (
                <li key={s.placeId} role="option" aria-selected={false}>
                  ...
                  >
                    <div className="grid h-12 w-12 shrink-0 place-items-center rounded-[13px] border border-[#d77bff]/26 bg-[#080808]/88">
                      <MapPin className="h-5 w-5 text-[#d77bff]" />
                    </div>
