  ...
  return (
    <section className="client-page-tight">
      <Link href="/dashboard/perfil" className="client-card block p-5 no-underline">
        <div className="flex items-start gap-4">
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2">
              <h2 className="min-w-0 flex-1 text-[20px] font-black text-[#f5f0e4]">Verificacao</h2>
              <span className="rounded-full border border-[#d4a843]/18 bg-[#d4a843]/10 px-2 py-0.5 text-[11px] font-bold text-[#f5d78c]">
                {doneCount}/{steps.length}
              </span>
            </div>
            <p className="mt-2 text-[14px] leading-6 text-[#f5f0e4]/54">
        ...
        <div className="mt-5 h-[5px] overflow-hidden rounded-full bg-white/10">
          <div
        ...
        <div className="mt-5 space-y-3">
          {steps.map((step) => (
            <div key={step.label} className="flex items-center gap-3 text-[14px]">
              {step.done ? (
